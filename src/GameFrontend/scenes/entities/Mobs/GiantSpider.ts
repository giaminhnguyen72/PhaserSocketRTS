...
export class GiantSpider implements Entity {
    ...
    constructor() {
        ...
        transform.vel.x = 0
        transform.vel.y = 0
        let sprite = new TimedSpriteSheet3d("/images/Characters/GiantSpider_S.png", {
            rot:0,
            dim:{
                length: 64,
                height:64
        ...
        script.setInit((system) =>{
            ...
            system.addSuperClasses(script, "Livable", "Moveable", "SpriteSheet", "Enemy", "GIANTSPIDER")
        ...
        // })
        let sync = new MultiplayerSyncronizer<GiantSpider, Data>(this, (data: Data) =>{
...
export class SpiderSystem implements ScriptOperable{
    constructor() {

    }
    update(dt: number, script:ScriptingEngine): void {
        let spiders = script.queryClass("GIANTSPIDER")
        if (spiders) {
            for (let i of spiders) {
                ...
                if (cooldown > 10000) {
                    let web = new Web()
                    web.components[1].pos.x =position.x
                    web.components[1].pos.y =position.y
                    i.setProperty("Cooldown", 0)
                    script.sceneManager.currScene.addEntity(web)
                } else {
